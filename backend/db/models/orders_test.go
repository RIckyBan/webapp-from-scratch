...
func testOrderToOnePhoneUsingPhone(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := OrderSlice{&local}
	if err = local.L.LoadPhone(ctx, tx, false, (*[]*Order)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testOrderToOneSetOpPhoneUsingPhone(t *testing.T) {
	var err error
...
var (
	orderDBTypes = map[string]string{`ID`: `varchar`, `UserID`: `varchar`, `PhoneID`: `varchar`, `Quantity`: `int`, `OrderDate`: `date`}
	_            = bytes.MinRead
)
