...
func testCartToOneUserUsingUser(t *testing.T) {
	...
	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &local, cartDBTypes, false, cartColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize Cart struct: %s", err)
	}
...
func testCartToOnePhoneUsingPhone(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err := randomize.Struct(seed, &foreign, phoneDBTypes, false, phoneColumnsWithDefault...); err != nil {
	...
	local.PhoneID = foreign.ID
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.Phone().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	ranAfterSelectHook := false
	AddPhoneHook(boil.AfterSelectHook, func(ctx context.Context, e boil.ContextExecutor, o *Phone) error {
		ranAfterSelectHook = true
		return nil
	})

	slice := CartSlice{&local}
	if err = local.L.LoadPhone(ctx, tx, false, (*[]*Cart)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.Phone == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.Phone = nil
	if err = local.L.LoadPhone(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.Phone == nil {
		t.Error("struct should have been eager loaded")
...
func testCartToOneSetOpPhoneUsingPhone(t *testing.T) {
	var err error
	...
	if err = randomize.Struct(seed, &b, phoneDBTypes, false, strmangle.SetComplement(phonePrimaryKeyColumns, phoneColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, phoneDBTypes, false, strmangle.SetComplement(phonePrimaryKeyColumns, phoneColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	...
	for i, x := range []*Phone{&b, &c} {
		err = a.SetPhone(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.PhoneID != x.ID {
			t.Error("foreign key was wrong value", a.PhoneID)
		}

		zero := reflect.Zero(reflect.TypeOf(a.PhoneID))
		reflect.Indirect(reflect.ValueOf(&a.PhoneID)).Set(zero)
		...
		if a.PhoneID != x.ID {
			t.Error("foreign key was wrong value", a.PhoneID, x.ID)
		}
	}
}
...
var (
	cartDBTypes = map[string]string{`ID`: `varchar`, `UserID`: `varchar`, `PhoneID`: `varchar`, `Quantity`: `int`}
	_           = bytes.MinRead
)
...
func testCartsUpsert(t *testing.T) {
	...
	if len(mySQLCartUniqueColumns) == 0 {
